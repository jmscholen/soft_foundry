...
- `lib/soft_foundry/advisory.rb` (new): `SoftFoundry::Advisory` computes go-live notices for a change record: review/judge waived through `skipped_phases`; with `surfaces.accessibility: true`, a specification without a `category: accessibility` requirement (or skipped), evaluation results with empty, placeholder, or N/A accessibility observations, and an accessibility review that is pending, skipped, still holding template placeholders, or N/A; with the flag false in a repository whose profile records a user-facing framework, a prompt to confirm; and a missing `.ai/rules/accessibility.md`.
