...
assert "REM plain run prints conflicts: naming the path" "sed -n '/STEP 2/,/STEP 3/p' '$LOG' | grep -q '^conflicts: .ai/rules/general.md\$'"
assert "REM plain run prints next: with git diff and --force" "sed -n '/STEP 2/,/STEP 3/p' '$LOG' | grep '^next: ' | grep -q 'git diff' && sed -n '/STEP 2/,/STEP 3/p' '$LOG' | grep '^next: ' | grep -q -- '--force'"
assert "REM --force run still prints conflicts: and next: (uncommitted edits are never overwritten)" "sed -n '/STEP 3/,\$p' '$LOG' | grep -q '^conflicts: .ai/rules/general.md\$' && sed -n '/STEP 3/,\$p' '$LOG' | grep '^next: ' | grep -q 'uncommitted edits are never overwritten'"
finish EVAL-005
